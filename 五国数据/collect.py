...
def start():
    global resstr
    os.chdir(path)
    resultfile = open("result.csv", 'w', encoding='gbk')
        ...
        if os.path.isdir(file) and not file.startswith('.'):
            ...
            for csvfile in filelist:
                ...
                f = open(path+"/"+file+"/"+csvfile, 'r',
                         encoding='gbk', errors='backslashreplace')
    ...
    resultfile.close()
    # block_size = 4096
    # with codecs.open("tmp.csv", 'r', source_encoding) as f1:
    #     with codecs.open("result.csv", 'wb') as f2:
    #         content = f1.read()
    #         outrow = content.encode('mbcs')
    #         f2.write(outrow)
...
b = tk.Button(window, text='点此按钮选择数据所在目录', font=(
    'Arial', 12), width=10, height=5, command=hit_me)
b.pack(fill=X, padx=20, pady=10)

l = tk.Label(window, textvariable=var, bg='gray', fg='white',
             font=('Arial', 12), width=30, height=2)
# 说明： bg为背景，fg为字体颜色，font为字体，width为长，height为高，这里的长和高是字符的长和高，比如height=2,就是标签有2个字符这么高
l.pack(fill=X, padx=20, pady=10)

# 第5步，在窗口界面设置放置Button按键
w = tk.Button(window, text='点此按钮开始处理数据', font=(
    'Arial', 12), width=30, height=5, command=start)
w.pack(fill=X, padx=20, pady=10)

r = tk.Label(window, textvariable=resstr, bg='gray',
